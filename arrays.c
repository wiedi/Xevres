...
unsigned long array_getfreeslot(array *a) {
  if ((a->cursi+1)>=a->maxsi) {
    ...
    if (a->content==NULL) {
      putlog("FATAL: Ran out of memory while resizing Array");
      fprintf(stderr,"FATAL: Ran out of memory while resizing Array\n");
      exit(1);
    }
  }
  a->cursi++;
...
void array_delslot(array *a, unsigned long slotn) {
  ...
  if ((a->cursi + a->arlim2) < a->maxsi) {
    ...
    a->content=realloc(a->content,a->maxsi*a->memberlen);
    if (a->content==NULL) {
      putlog("FATAL: Ran out of memory while making Array smaller?!");
      fprintf(stderr,"FATAL: Ran out of memory while making Array smaller?!\n");
      exit(1);
    }
  }
}
